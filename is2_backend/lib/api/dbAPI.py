...
sys.path.append(os.path.abspath(os.path.join(
    os.path.dirname(__file__), '../data_base')))
...
def register_user():
    ...
    new_user = User(name=name, email=email,
                    password=hashed_password, type_of_user='user')
...
def login():
    ...
    if user is None:
        response = make_response(
            jsonify({"error": "Correo o contraseña incorrectas"}), 401)
        return response
    if not bcrypt.check_password_hash(user.password, password):
        response = make_response(
            jsonify({"error": "Contraseña incorrecta"}), 401)
        return response
...
def create_user():
    ...
    hashed_password = bcrypt.generate_password_hash(password)
    new_user = User(name=name, email=email,
                    password=hashed_password, type_of_user=type_of_user)
    db.session.add(new_user)
    db.session.commit()
...
def create_dev():
    ...
    password = request.json['password']
    hashed_password = bcrypt.generate_password_hash(password)
    new_dev = Developer(name=name, email=email,
                        password=hashed_password, type_of_user='developer')
...
def create_admin():
    ...
    new_admin = Admin(name=name, email=email,
                      password=hashed_password, type_of_user='admin')
...
def create_comment():
    if request.method == "OPTIONS":  # CORS preflight
        ...
        new_comment = Comment(content=content, commenter_id=commenter_id,
                              commenter_name=commenter_name, report_id=report_id)
...
def get_software_reports(id):
    ...
    if reports:
        report_list = []
        for report in reports:
            report_list.append({'id': report.id, 'title': report.title, 'description': report.description, 'date': report.date.isoformat(
            ), 'user_id': report.user_id, 'urgency': report.urgency, 'status': report.status})
...
def count_reports_by_software(email):
    ...
    for report in reports:
        ...
        software_obj = next(
            (obj for obj in software_objects if obj["name"] == software_name), None)

        if software_obj is None:
            software_obj = {"name": software_name,
                            "jobsDone": 0, "totalJobs": 0}
